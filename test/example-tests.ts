...
describe("examples", () => {
  ...
  it("can even do parameter-less lambda's", () => {
    ...
    scope.filter = (
      items: JsExpressionWrappedValue<any[]>,
      variableName: string,
      callback: () => boolean
    ) => {
  ...
  it("can use functions provided in the scope that take wrappedValues as parameter", () => {
    let context = createDefaultJsExpressionContext({
      param: { num: 5, exp: 2 },
      square: (bigNum: JsExpressionWrappedValue<{ num: number; exp: number }>) => ({
        num: bigNum.wrapped.num * bigNum.wrapped.num,
        exp: bigNum.wrapped.exp,
      }),
    });
  ...
  it("can even evaluate hyperscript", () => {
    ...
    let scope = {
      h: (tagName: string, children: JsExpressionWrappedValue<string[]>) => ({
        tagName,
